  ...
  <div id="trend">
    ...
    <div style="padding: 20px 10px 20px 10px;">
      <scroller lock-y :scrollbar-x=false>
        <div class="box1">
          <x-table class="box1-item" v-for="item in 1" :key="item">
...
import {Scroller ,Spinner,LoadMore,ButtonTab, ButtonTabItem,XTable,PopupPicker, 
        Group, Cell, Picker, XButton } from 'vux'
...
export default {
  ...
  data () {
   return {
      ...
      lupdate:0,
      title: '趋势图',
      list: ['登记率', '可售', '通过率', '项目可售','登记率', '可售', '通过率', '项目可售'],
      value: ['汇总_GMV'],
  ...
  methods: {
    ...
    getList(item,index){
      ...
      //选择列表
     this.$http.fetch('dsa/dataBoard/indTnd/seList',
                    {        
                    "uid":uid,
                    "uname":uname,
                    "dataType":dataType
        ...
        //分页查询列表    
       this.$http.fetch('dsa/dataBoard/indTnd/pageList',
                       {        
                        "uid":uid,
                        "uname":uname,
                        "dataType":dataType,
                    ...
                    if(response.data.status == 200){
                      if(response.data.result.rows.length > 0){
                        // 使用 html5 canvas api 创建渐变色对象
                          ...
                          let position = ['7日', 98.88]
                          let content = '最大值：'+98.88
                          let data = [
                            { 日期: "1日", 该指标值: 7.2 },
                            { 日期: "2日", 该指标值: 27.5 },
                            { 日期: "3日", 该指标值:67.8 },
...
x-table{background: #FFFFFF;border: 1px solid #EEEEEE;}
